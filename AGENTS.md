...
`imv`는 NovelAI와 ComfyUI 이미지의 메타데이터를 로컬에서 색인·검색·내보내기·정리하는 CLI 우선 도구다. 현재 개발의 중심은 설치 부담이 낮은 Go CLI와 공통 코어이며, GUI는 같은 코어를 사용하는 얇은 독립 데스크톱 어댑터로만 유지한다.
...
- GUI는 브라우저 우선 제품으로 전환하지 않는다. 다시 적극 개발할 때도 Wails 기반 독립 데스크톱 앱을 기본으로 한다.
...
6. 사용자 동작이나 설계 결정이 달라졌으면 관련 문서와 ADR을 함께 갱신한다.

## 기본 검증 / Default verification

Windows에서는 전역 Go가 없으면 `.tools/go-portable-1.26.4/go/bin/go.exe`를 사용하고 `GOMODCACHE`와 `GOCACHE`를 작업공간 내부로 지정한다. 완료 전 최소 검증은 다음과 같다.
...
GUI 또는 공통 API를 수정했다면 `go build ./cmd/imv-gui`도 실행한다. `gui`나 임베드 산출물을 수정한 경우에만 Node/Vite 검증을 추가한다.
